...
@test "check limit passed in query" {
  run steampipe query "select limit_value from chaos_limit_verify_rows_remaining where row_count=1 limit 10" --output=json
  limit=$(echo $output | jq .[].limit_value)
  assert_equal "$limit" "10"
}

@test "check limit returns null when distinct passed in query" {
  run steampipe query "select distinct limit_value from chaos_limit_verify_rows_remaining where row_count=1 limit 10" --output=json
  limit=$(echo $output | jq .[].limit_value)
  assert_equal "$limit" "null"
}

@test "check limit returns null when order by passed in query" {
  run steampipe query "select limit_value from chaos_limit_verify_rows_remaining order by row_count limit 10" --output=json
...
@test "check limit returns null when there is a key column passed in query but with wrong operator" {
  run steampipe query "select limit_value from chaos.chaos_limit_verify_rows_remaining where row_count > 4 limit 10" --output=json
...
@test "check limit when a key column passed in query" {
  ...
  # limit is not returned as null since row_count is a key column 
  limit=$(echo $output | jq .[0].limit_value)
  assert_equal "$limit" "10"
}
...
@test "check rows remaining when a limit is passed in query" {
  run steampipe query "select limit_value, rows_streamed, sdk_rows_remaining from chaos_limit_verify_rows_remaining where row_count=2 limit 10" --output=json
  echo $output
...
@test "check rows remaining when no limit is passed in query" {
  run steampipe query "select limit_value, rows_streamed, sdk_rows_remaining, row_count from chaos_limit_verify_rows_remaining where row_count=10" --output=json
  echo $output
  # Use jq command-line tool to parse the JSON string
  object_count=$(echo "$output" | jq length)
  ...
  for ((i=0; i<object_count; i++)); do
    ...
    rows_streamed=$(echo "$object" | jq --raw-output '.rows_streamed')
    sdk_rows_remaining=$(echo "$object" | jq --raw-output '.sdk_rows_remaining')
    row_count=$(echo "$object" | jq --raw-output '.row_count')
    sum=$((sdk_rows_remaining + rows_streamed))
